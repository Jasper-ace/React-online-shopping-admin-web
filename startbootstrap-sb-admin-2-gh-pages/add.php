...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Validate and sanitize 'productPrice' input
    $productPrice = ($productPrice !== '') ? filter_var($productPrice, FILTER_VALIDATE_FLOAT) : null;

    if ($productPrice === false) {
        echo "Invalid Product Price value";
        exit;
    }

    // Calculate total
    $productTotal = $productPrice * $productStock;

    // Construct the SQL query for product insertion
    $insertProductSQL = "INSERT INTO products (ProductName, Price, Size, Stocks, Total)
                        VALUES ('$productName', $productPrice, '$productSize', $productStock, $productTotal)";
    ...
    if ($conn->query($insertProductSQL) === TRUE) {
        ...
        // Handle file upload for product photo only if a file is selected
        if ($_FILES["productPhoto"]["tmp_name"] !== "") {
            ...
            if ($uploadOk == 0) {
                ...
                if (move_uploaded_file($_FILES["productPhoto"]["tmp_name"], $targetFile)) {
                    // Insert photo path into the product_photos table
                    $insertPhotoSQL = "INSERT INTO product_photos (product_id, photo_path)
                                        VALUES ($lastProductId, '$targetFile')";
                    $conn->query($insertPhotoSQL);
                    echo "<script>alert('Product and Photo inserted successfully');</script>";
                } else {
